...
void main() {
  ...
  group('TLS Error Tests - Untrusted Certificate', () {
    ...
    testWidgets('HTTPS request fails with untrusted certificate and MITM enabled', (WidgetTester tester) async {
      ...
      try {
        ...
        try {
          ...
          expect(response.statusCode, isIn(const [200, 301, 302, 400, 403, 502]));
        } catch (e) {
          // Expected: HandshakeException or SocketException or CertificateException
          final errorType = e.runtimeType.toString();
          expect(errorType, anyOf(contains('HandshakeException'), contains('SocketException'), contains('CertificateException')));
        }
    ...
    testWidgets('HTTPS request with rejected certificate callback captures exchange with error', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
    ...
    testWidgets('Multiple consecutive requests with untrusted cert are handled', (WidgetTester tester) async {
      final client = createUntrustedClient();
      ...
      for (var i = 0; i < 3; i++) {
        try {
          final request = await client.openUrl('GET', Uri.parse('https://httpbin.org/get?test=$i'));
          await request.close();
  ...
  group('TLS Error Tests - Blind Tunnel (Non-Configured Domains)', () {
    ...
    testWidgets('HTTPS request to non-configured domain uses blind tunnel', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        expect(response.statusCode, isIn(const [200, 301, 302, 403, 407, 502, 503, 504]));
        
        // Wait for exchange to be captured
        await Future.delayed(const Duration(milliseconds: 1500));
        
        // Find the exchange for google.com
        final googleExchanges = exchanges.where((e) => 
          e.url.contains('google.com') || e.host.contains('google.com')
        );
        
        expect(googleExchanges.length, greaterThanOrEqualTo(1), 
    ...
    testWidgets('HTTPS request to configured domain with MITM enabled', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        try {
          final request = await client.openUrl('GET', Uri.parse('https://configured-domain.test'));
          final response = await request.close();
          // May succeed or fail depending on DNS and CA trust
          expect(response.statusCode, isIn(const [200, 301, 302, 404, 502, 503, 504]));
        } catch (e) {
        ...
        final domainExchanges = exchanges.where((e) => 
          e.url.contains('configured-domain.test') || e.host.contains('configured-domain.test')
        );
    ...
    testWidgets('Blind tunnel does not decrypt request body', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final request = await client.openUrl('POST', Uri.parse('https://www.google.com/upload'));
        request.headers.set('Content-Type', 'text/plain');
        request.write('test data for blind tunnel');
        final response = await request.close();
        
        // Should succeed (tunnel created)
        expect(response.statusCode, isIn(const [200, 301, 302, 400, 403, 404, 405, 502]));
        
        await Future.delayed(const Duration(milliseconds: 1500));
        
        // Find the exchange
        final googleExchanges = exchanges.where((e) => 
          e.method == 'POST' && (e.url.contains('google.com') || e.host.contains('google.com'))
        );
  ...
  group('TLS Error Tests - CA Not Initialized', () {
    ...
    testWidgets('Proxy with MITM disabled does not initialize CA', (WidgetTester tester) async {
    ...
    testWidgets('CA trust check returns false in test environment', (WidgetTester tester) async {
  ...
  group('TLS Error Tests - Connection Failures', () {
    ...
    testWidgets('Connection timeout is handled gracefully', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        try {
          ...
          final request = await client.openUrl('GET', Uri.parse('https://this-domain-should-not-exist-12345.test'));
          await request.close();
        } catch (e) {
          caughtTimeout = true;
          // Expected: timeout or connection error
          expect(e.toString(), anyOf(contains('timeout'), contains('Connection timed out'), contains('failed')));
        }
    ...
    testWidgets('Connection refused to closed port is handled', (WidgetTester tester) async {
    ...
    testWidgets('Invalid HTTPS URL is handled gracefully', (WidgetTester tester) async {
      final client = HttpClient();
      client.findProxy = (uri) {
        return "PROXY 127.0.0.1:$proxyPort";
      };
  ...
  group('TLS Error Tests - Error Reporting in Exchanges', () {
    ...
    testWidgets('Failed exchanges have error state', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        try {
          final request = await client.openUrl('GET', Uri.parse('https://non-existent-domain-54321.test'));
          await request.close();
        } catch (e) {
          // Expected to fail
        }
        ...
        final failedExchanges = exchanges.where((e) => e.state == ExchangeState.failed);
    ...
    testWidgets('Exchange state transitions are captured', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        try {
          final request = await client.openUrl('GET', Uri.parse('https://httpbin.org/get'));
          final response = await request.close();
          expect(response.statusCode, isIn(const [200, 301, 302, 400, 403, 502, 503, 504]));
        } catch (e) {
        ...
        final completedExchanges = exchanges.where((e) => e.state == ExchangeState.completed);
    ...
    testWidgets('Exchange error message is captured for failed requests', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final exchangesWithErrors = exchanges.where((e) => e.errorMessage != null);
