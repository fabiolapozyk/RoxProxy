...
void main() {
  ...
  group('MITM Decryption Tests - With CA Installation', () {
    ...
    testWidgets('HTTPS request is MITM decrypted when CA is installed', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        while (stopwatch.elapsedMilliseconds < 5000) {
          final httpsExchanges = exchanges.where((e) => 
            e.url.contains('example.com') && e.isMITMDecrypted
          );
          if (httpsExchanges.isNotEmpty) {
            break;
          }
        ...
        expect(exchanges.length, greaterThan(0), reason: 'Expected exchange to be captured');
    ...
    testWidgets('MITM decrypted request has captured headers', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        if (httpsExchanges.isNotEmpty) {
          final exchange = httpsExchanges.first;
          expect(exchange.requestHeaders, isNotEmpty, reason: 'Expected request headers to be captured');
          
          // If decrypted, we should see our custom header
          if (exchange.isMITMDecrypted) {
            final headerNames = exchange.requestHeaders.map((h) => h.name.toLowerCase()).toList();
            expect(headerNames, contains('x-mitm-test'), 
                reason: 'Expected custom header to be captured in decrypted request');
          }
        }
    ...
    testWidgets('MITM decrypted request body is captured', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final request = await client.openUrl('POST', Uri.parse('https://httpbin.org/post'));
        ...
        while (stopwatch.elapsedMilliseconds < 5000) {
          final postExchanges = exchanges.where((e) => 
            e.method == 'POST' && e.url.contains('httpbin.org') && e.requestBodyRef != null
          );
        ...
        expect(exchangeWithBody, isNotNull, 
        ...
        if (exchangeWithBody != null) {
          ...
          if (exchangeWithBody.isMITMDecrypted && exchangeWithBody.requestBodyRef != null) {
            final bodyBytes = await proxyChannel.fetchBody(exchangeWithBody.requestBodyRef!);
    ...
    testWidgets('MITM decrypted response body is captured', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final request = await client.openUrl('GET', Uri.parse('https://httpbin.org/json'));
        final response = await request.close();
        expect(response.statusCode, equals(200));
        ...
        while (stopwatch.elapsedMilliseconds < 5000) {
          final getExchanges = exchanges.where((e) => 
            e.method == 'GET' && e.url.contains('httpbin.org') && e.responseBodyRef != null
          );
        ...
        expect(exchangeWithResponse, isNotNull, 
        ...
        if (exchangeWithResponse != null) {
          ...
          if (exchangeWithResponse.isMITMDecrypted && exchangeWithResponse.responseBodyRef != null) {
            final bodyBytes = await proxyChannel.fetchBody(exchangeWithResponse.responseBodyRef!);
            expect(bodyBytes, isNotNull);
            expect(bodyBytes!.length, greaterThan(0));
    ...
    testWidgets('MITM decrypted response has captured headers', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        // Make HTTPS request
        final request = await client.openUrl('GET', Uri.parse('https://httpbin.org/headers'));
        final response = await request.close();
        expect(response.statusCode, equals(200));
        ...
        if (exchange.isMITMDecrypted) {
          expect(exchange.responseHeaders, isNotNull, reason: 'Expected response headers for MITM decrypted exchange');
          expect(exchange.responseHeaders!.isNotEmpty, isTrue);
          
          // Check for common response headers
          final headerNames = exchange.responseHeaders!.map((h) => h.name.toLowerCase()).toList();
    ...
    testWidgets('Non-configured HTTPS domains use blind tunnel (not decrypted)', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final tunnelExchanges = exchanges.where((e) => 
    ...
    testWidgets('MITM decryption works with wildcard domain rules', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final request = await client.openUrl('GET', Uri.parse('https://test.example.com'));
        final response = await request.close();
        ...
        // Find the exchange
        final wildcardExchanges = exchanges.where((e) => 
          e.url.contains('test.example.com') || e.host.contains('test.example.com')
        );
