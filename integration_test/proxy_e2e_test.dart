...
void main() {
  ...
  group('Proxy E2E Tests - HTTP', () {
    ...
    testWidgets('Proxy intercepts HTTP GET requests', (
      WidgetTester tester,
    ) async {
      // Capture exchanges
      ...
      try {
        ...
        expect(
          exchanges.length,
          greaterThan(0),
          reason: 'Expected at least one exchange to be captured',
        );
        ...
        expect(
          getExchanges.length,
          greaterThan(0),
          reason: 'Expected GET request to be captured',
        );
    ...
    testWidgets('Proxy captures request headers', (WidgetTester tester) async {
      ...
      try {
        ...
        if (getExchanges.isNotEmpty) {
          ...
          expect(
            customHeader.name.isNotEmpty,
            isTrue,
            reason: 'Expected custom header to be captured',
          );
        }
      } finally {
        await subscription.cancel();
    ...
    testWidgets('Proxy handles HTTP POST requests', (
      WidgetTester tester,
    ) async {
      // Capture exchanges
      final exchanges = <CapturedExchange>[];
      ...
      try {
        ...
        expect(
          postExchanges.length,
          greaterThan(0),
          reason: 'Expected POST request to be captured',
        );
      ...
      try {
        ...
        while (stopwatch.elapsedMilliseconds < 3000) {
          final getExchanges = exchanges.where(
            (e) => e.method == 'GET' && e.statusCode != null,
          );
          if (getExchanges.isNotEmpty) {
        ...
        expect(
          exchangeWithStatus,
          isNotNull,
          reason: 'Expected exchange with statusCode to be captured',
        );
  ...
  group('Proxy E2E Tests - HTTPS', () {
    ...
    testWidgets('Proxy handles HTTPS CONNECT requests', (
      WidgetTester tester,
    ) async {
      // Capture exchanges
      final exchanges = <CapturedExchange>[];
    ...
    testWidgets('Proxy creates tunnel for HTTPS requests', (
      WidgetTester tester,
    ) async {
  ...
  group('Proxy E2E Tests - Real Servers', () {
    ...
    testWidgets('Proxy forwards requests to httpbin.org', (
      WidgetTester tester,
    ) async {
      final client = HttpClient();
      client.findProxy = (uri) {
    ...
    testWidgets('Proxy captures request and response bodies', (
      WidgetTester tester,
    ) async {
      final exchanges = <CapturedExchange>[];
    ...
    testWidgets('Proxy captures different HTTP status codes', (
      WidgetTester tester,
    ) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
      ...
      try {
        ...
        // Test 200 OK
        final response200 = await client.openUrl(
          'GET',
        ...
        // Test 404 Not Found
        final response404 = await client.openUrl(
          'GET',
        ...
        final response500 = await client.openUrl(
          'GET',
          Uri.parse('http://httpbin.org/status/500'),
        );
        ...
        expect(
          status200Exchanges.length,
          greaterThan(0),
          reason: 'Expected 200 status to be captured',
        );
        expect(
          status404Exchanges.length,
          greaterThan(0),
          reason: 'Expected 404 status to be captured',
        );
        expect(
          status500Exchanges.length,
          greaterThan(0),
          reason: 'Expected 500 status to be captured',
        );
    ...
    testWidgets('Proxy captures status message along with code', (
      WidgetTester tester,
    ) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
      ...
      try {
        ...
        final response = await client.openUrl(
          'GET',
          Uri.parse('http://httpbin.org/get'),
        );
        ...
        final getExchanges = exchanges.where(
          (e) => e.method == 'GET' && e.statusCode == 200,
        );
    ...
    testWidgets('Proxy handles redirect status codes', (
      WidgetTester tester,
    ) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
      ...
      try {
        ...
        final response = await client.openUrl(
          'GET',
          Uri.parse('http://httpbin.org/relative-redirect/1'),
        );
        final resp = await response.close();
        ...
        final redirectExchanges = exchanges.where(
          (e) =>
              e.statusCode != null &&
              e.statusCode! >= 300 &&
              e.statusCode! < 400,
    ...
    testWidgets('Proxy captures 4xx client error status codes', (
      WidgetTester tester,
    ) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
      ...
      try {
        ...
        final response400 = await client.openUrl(
          'GET',
          Uri.parse('http://httpbin.org/status/400'),
        );
        final resp400 = await response400.close();
        expect(resp400.statusCode, equals(400));

        // Test 403 Forbidden
        final response403 = await client.openUrl(
          'GET',
          Uri.parse('http://httpbin.org/status/403'),
        );
        ...
        final clientErrorExchanges = exchanges.where(
          (e) =>
              e.statusCode != null &&
              e.statusCode! >= 400 &&
              e.statusCode! < 500,
