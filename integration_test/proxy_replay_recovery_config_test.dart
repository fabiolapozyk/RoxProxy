...
void main() {
  ...
  group('Replay Tests', () {
    ...
    testWidgets('Replay captured request creates new exchange', (WidgetTester tester) async {
      ...
      try {
        ...
        final request1 = await client.openUrl('GET', Uri.parse('https://httpbin.org/get'));
        ...
        final originalExchanges = exchanges.where((e) => e.url.contains('httpbin.org/get'));
        ...
        // Replay the request
        final newExchangeId = await proxyChannel.replayRequest(replayRequest.toMap());
        ...
        final allExchanges = exchanges.where((e) => e.url.contains('httpbin.org/get'));
    ...
    testWidgets('Replay request with modified headers', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
      ...
      try {
        ...
        final originalExchanges = exchanges.where((e) => e.url.contains('httpbin.org'));
        ...
        final allExchangesAfterReplay = exchanges.where((e) => e.url.contains('httpbin.org/headers'));
    ...
    testWidgets('Replay request with body', (WidgetTester tester) async {
      ...
      try {
        ...
        final request1 = await client.openUrl('POST', Uri.parse('https://httpbin.org/post'));
  ...
  group('Recovery Tests', () {
    ...
    testWidgets('Proxy can be stopped and restarted', (WidgetTester tester) async {
      proxyChannel = ProxyChannel();
    ...
    testWidgets('Proxy can be started on different ports sequentially', (WidgetTester tester) async {
      proxyChannel = ProxyChannel();
    ...
    testWidgets('Multiple proxy instances can be managed', (WidgetTester tester) async {
      ...
      final port2 = await proxy2.startProxy(
        port: 20000,
        domainRules: [
          DomainRule(domain: 'example.org', isEnabled: true),
        ],
        connectionTimeoutSeconds: 30,
        setSystemProxy: false,
        httpsInterceptionEnabled: false,
  ...
  group('Configuration Tests', () {
    ...
    setUpAll(() async {
      ...
      proxyPort = await proxyChannel.startProxy(
        port: 19995,
        domainRules: [
          DomainRule(domain: 'initial.com', isEnabled: true),
        ],
        connectionTimeoutSeconds: 30,
        setSystemProxy: false,
        httpsInterceptionEnabled: false,
    ...
    testWidgets('Proxy restarts when domain rules change', (WidgetTester tester) async {
      final exchanges = <CapturedExchange>[];
      final subscription = exchangeStream.listen((event) {
        exchanges.add(event.exchange);
      });
    ...
    testWidgets('Proxy with MITM disabled still runs but may not initialize CA', (WidgetTester tester) async {
    ...
    testWidgets('Connection timeout configuration is applied', (WidgetTester tester) async {
      ...
      try {
        final request = await client.openUrl('GET', Uri.parse('http://example.com'));
        final response = await request.close();
        
        // Should get a response (200, redirect, or proxy error)
        expect(response.statusCode, isIn(const [200, 301, 302, 304, 307, 308, 400, 403, 404, 407, 502, 504]));
